...
################revise by Lilin at 2016-3-24
#we keeped R1 which is longer than 36bp after TSO removement,so "$read_length" is not equal ,then $total_bases != $total_reads * $read_length;
...
my $UP1     = "CCGGCGCGCCGTCGACTTTTTTTTTT";      
...
open LOG,">$outdir/$sample.QC.log" or die $!;
...
my $add_log = "$outdir/$sample.scRNA_Seq_primer_trimming.log" if $scRNA;

# claim an array for parameters transmission in additional log composition
my @stats if $scRNA;

# primer stats vavible in primer trimmming
my ($UP1_count_in_r1, $UP2_count_in_r1, $polyA_in_r1, $UP1_count_in_r2, $UP2_count_in_r2, $polyA_in_r2) = (0) x 6 if $scRNA;        
...
chomp (my $file = "$indir/$sample.fq.gz");
...
open OUT_1,">:gzip","$outdir/$sample.clean.fq.gz" or die $!;
...
while (1) {
	...
	$total_bases += length($line2);    ###Lilin
	#-remove adapter
...
my $GC_figure = <<FIGURE;
	gc<-read.table("$outdir/$sample.ATGC")
	site<-gc[,1]
	base_a<-gc[,4]
	base_t<-gc[,7]
	base_g<-gc[,10]
	...
	pdf("$outdir/$sample.ATGC.pdf",width=8,height=6)
...
my $meanQ_errorR = <<FIGURE;
	table<-read.table("$outdir/$sample.mean_quality")
        site<-table[,1]
        quality<-table[,2]
        error<-table[,3]
        total_sites<-$X_axis
        pdf("$outdir/$sample.mean_quality.pdf",width=8,height=6)
        ...
        pdf("$outdir/$sample.ErrorRate.pdf",width=8,height=6)
...
sub error_rate {
	my($end) = @_;
	open OUT_3,">$outdir/$sample.mean_quality" or die $!;
...
sub gc_content {
	my($end) = @_;
	open OUT_4,">$outdir/$sample.ATGC" or die $!;
...
#	$total_bases = $total_reads * $read_length;       ###Lilin
