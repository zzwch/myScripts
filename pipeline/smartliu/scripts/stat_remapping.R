...
nArgs <- length(Args)
if(nArgs != 9){
  cat("Example: Rscript stat_remapping.R summary_dir mapping_dir samples reref")
...
for(s in samples){
  align_log <- file.path(mapping_dir, paste0(s,".unmapped.",reref,".align_summary.txt"))
...
write.table(alignStat, file = file.path(summary_dir, paste0("stat_",basename(mapping_dir),".remapping_to_",reref,".txt")), sep = "\t", row.names = F)
